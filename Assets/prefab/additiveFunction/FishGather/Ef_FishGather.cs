...
public class Ef_FishGather : MonoBehaviour {
    ...
    void Awake()
    {
        ...
        if (mFg == null)
        {
            Debug.LogError("Ef_FishGather 需要和 FishGather放在同一个脚本.");
            return;
        }
        ...
        foreach(Player p in GameMain.Singleton.Players)
        {
            ...
            Transform tsCreditBoard = p.transform.FindChild("UI_CreditBoard");
            if (tsCreditBoard == null)
            {
                Debug.LogError("GameObject.Player下必须有名为UI_CreditBoard的Go");
                continue;
            }
